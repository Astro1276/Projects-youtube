# 🐺 ¡Bienvenido a la Comunidad de Lobo Weaver! ✨

<div align="center">
  <img src="https://img.shields.io/badge/YouTube_Shorts-FF0000?style=for-the-badge&logo=youtube&logoColor=white" alt="YouTube Shorts">
  <img src="https://img.shields.io/badge/Lobo_Weaver-Comunidad-orange?style=for-the-badge&logo=github&logoColor=white" alt="Comunidad">
  <img src="https://img.shields.io/badge/Code_Design-Aesthetic-ff69b4?style=for-the-badge&logo=visual-studio-code&logoColor=white" alt="Estilo">
</div>

<br />

¡Qué onda, Weaver! 🐺 Bienvenido a la base de operaciones. Aquí no solo vienes por código, vienes a ser parte de una manada de creadores. En este repo tienes acceso a todos los experimentos visuales que ves en mis **YouTube Shorts**.

---

## 🚀 Comunidad & Interacción

¡No te quedes solo con el código! Nuestro foro de **GitHub Discussions** es el corazón de la manada. Haz clic en lo que necesites:

| 📢 [¡Ver Spoilers y Avisos!](https://github.com) | 🙋 [¿El código te dio error?](https://github.com) |
| :--- | :--- |
| **✨ [¡Presume tu diseño editado!](https://github.com)** | **💡 [Propón ideas para videos](https://github.com)** |
| **📊 [Vota en las encuestas](https://github.com)** | **🏆 [Hall of Fame (Top Weaver)](https://github.com)** |

---

## 📂 ¿Cómo descargar los archivos?

Para que sea más fácil, **cada proyecto es un solo archivo**:

1. **Busca el archivo**: El nombre del archivo coincide con el título del video (Ejemplo: `Boton_Neon_Glow.html`).
2. **Todo en uno**: Dentro de ese mismo archivo encontrarás el **HTML, CSS y JS** integrado. Solo copiar y pegar.
3. **Personaliza**: Cambia los colores, las fuentes y hazlo tuyo.

---

## 🛡️ Licencia y Uso (Anti-Copia)

Este proyecto está bajo la licencia **CC BY-NC-SA 4.0**.
* ✅ **Permitido:** Usar el código para aprender y en tus proyectos personales sin fines de lucro.
* ❌ **Prohibido:** No puedes vender este código ni usarlo para crear herramientas de pago o dependencia sin autorización.
* ⚠️ **Mención:** Si lo usas en tus redes, menciona a **Lobo Weaver**. ¡Respetemos el trabajo del creador!
* 📜 **Legal:** [Leer política completa de Lobo Weaver (LICENSE.md)](license.md)

---

## 🌸 Mis Redes & Soporte

Si quieres ver más procesos de diseño y estar al tanto de lo que viene:

* 📺 **[Mi Canal de YouTube](https://youtube.com)** — Suscríbete para los mejores Shorts de código.
* 📸 **[Mi Instagram](https://instagram.com)** — Detrás de cámaras y setups de un Weaver.
* ☕ **[Invítame un Café](https://buymeacoffee.com)** — Apoya el contenido gratuito si estos códigos te han servido.

---

<div align="center">
  <h3>💕 ¿Te sirvió el proyecto? ¡Regálame una ⭐ Star!</h3>
  <p>Al darle a la estrella, ayudas a que la manada de Lobo Weaver crezca y llegue a más programadores.</p>
</div>
